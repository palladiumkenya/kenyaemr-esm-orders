...
const procedureReportSchema = z.object({
  procedureReport: z.string({ required_error: 'Procedure report is required' }).min(1, {
    message: 'Procedure report is required',
  }),
});

type ProcedureReportFormData = z.infer<typeof procedureReportSchema>;
...
const ImagingReportForm: React.FC<ResultFormProps> = ({
  ...
  const {
    ...
    control,
    handleSubmit,
  } = useForm<ProcedureReportFormData>({
    defaultValues: {
      procedureReport: '',
    },
    resolver: zodResolver(procedureReportSchema),
    mode: 'all',
  });
  ...
  const onSubmit = async (formData: ProcedureReportFormData) => {
    const reportPayload = {
      patient: patientUuid,
  ...
  return (
    <>
      ...
      <form aria-label="sample form" className={styles.form} onSubmit={handleSubmit(onSubmit)}>
